...
describe("strings.normalize_slack_mention", function()
	it("converts <@U12345678|user> to @user", function()
		assert.same("@user", strings.normalize_slack_mention("<@U12345678|user>"))
	end)
	-- <@U01SD385GE6|tatsuhiro-yamashita>
	it("converts <@U01SD385GE6|sei-mei> to @sei-mei", function()
		assert.same("@sei-mei", strings.normalize_slack_mention("<@U01SD385GE6|sei-mei>"))
	end)
	it("does not convert @user", function()
		assert.same("@user", strings.normalize_slack_mention("@user"))
	end)
	it("does not convert plaintext", function()
		assert.same("plaintext", strings.normalize_slack_mention("plaintext"))
	end)
end)
